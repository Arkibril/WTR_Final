...
public class DataPersistenceManager : MonoBehaviour
{
    ...
    public CinematiqueData cinematiqueData;
    private List<IDataPersistence<CinematiqueData>> cinematiqueDataObjects;
    ...
    public void NewCinematiqueData()
    {
        this.cinematiqueData = new CinematiqueData();
    }

    public void LoadCinematiqueData()
    {
        if (dataHandler == null)
        {
            Debug.LogError("DataHandler is not assigned. Cannot load CinematiqueData.");
            return;
        }

        this.cinematiqueData = dataHandler.Load<CinematiqueData>();

        if (this.cinematiqueData == null)
        {
            Debug.Log("Aucun data n'a été trouvé. Initialisation des data par défauts");
            NewCinematiqueData();
        }

        foreach (IDataPersistence<CinematiqueData> dataPersistenceObj in cinematiqueDataObjects)
        {
            dataPersistenceObj.LoadData(cinematiqueData);
        }
    }

    public void SaveCinematiqueData()
    {
        if (dataHandler == null)
        {
            Debug.LogError("DataHandler is not assigned. Cannot save CinematiqueData.");
            return;
        }

        foreach (IDataPersistence<CinematiqueData> dataPersistenceObj in cinematiqueDataObjects)
        {
            dataPersistenceObj.SaveData(ref cinematiqueData);
        }

        dataHandler.Save(cinematiqueData);
    }
    ...
    private List<IDataPersistence<CinematiqueData>> FindAllCinematiqueData()
    {
        MonoBehaviour[] monoBehaviours = FindObjectsOfType<MonoBehaviour>();
        List<IDataPersistence<CinematiqueData>> cinematiqueDataObjects = new List<IDataPersistence<CinematiqueData>>();

        foreach (var monoBehaviour in monoBehaviours)
        {
            if (monoBehaviour is IDataPersistence<CinematiqueData> cinematiqueDataObj)
            {
                cinematiqueDataObjects.Add(cinematiqueDataObj);
            }
        }

        return cinematiqueDataObjects;
    }
}
