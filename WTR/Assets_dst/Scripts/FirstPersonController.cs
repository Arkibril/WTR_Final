...
public class FirstPersonController : MonoBehaviour
{
    ...
    [Range(1f, 100f)] [SerializeField] public float smoothRotateSpeed = 5f;
    ...
    protected virtual void SmoothSpeed()
    {
        ...
        if (movementInputData.IsRunning && CanRun() && m_Player.m_sprint)
        {
            float _walkRunPercent = Mathf.InverseLerp(walkSpeed, sprintSpeed, m_smoothCurrentSpeed);
            m_finalSmoothCurrentSpeed = runTransitionCurve.Evaluate(_walkRunPercent) * m_walkRunSpeedDifference + runSpeed;
        }
        else if (movementInputData.IsRunning && CanRun() && m_Player.m_run)
        {
    ...
    public bool CanRun()
    {
        ...
        return _dot >= canRunThreshold && !movementInputData.IsCrouching && !m_Player.m_walk ? true : false;
    }
